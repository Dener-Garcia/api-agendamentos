...
async function getUserAdmin(req, res) {
    ...
    const user = await serviceUsers.getUserAdmin(email, password)
...
async function insertUserAdmin(req, res) {

    const { name, email, password } = req.body

    const user = await serviceUsers.insertUserAdmin(name, email, password)
    res.status(201).json(user)
